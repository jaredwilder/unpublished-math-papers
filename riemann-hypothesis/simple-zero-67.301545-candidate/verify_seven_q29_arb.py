...
GRID = 4_000
PRECISION_BITS = 128
TARGET_NUMERATOR = 341
TARGET_DENOMINATOR = 100_000
PRESSURE_NUMERATOR = 29
PRESSURE_DENOMINATOR = 100_000
# ceil((TARGET/PRESSURE) * GRID) = ceil((341/29)*4000)
PRESSURE_CUTOFF_CELLS = 47_035
...
def verify_seven(progress_every: int = 0) -> VerificationReport:
    r"""Prove F6_q(g1,...,g6) >= 341/100000 for all nonnegative gaps,
    with q = 29/100000.
    ...
    # U(g)=(29/100000)g+w(g)/3 is a term of F6_q. Any cell on which U already
    ...
    for index in range(PRESSURE_CUTOFF_CELLS):
        one_body = down_ratio(PRESSURE_NUMERATOR * index, GRID * PRESSURE_DENOMINATOR)
    ...
    def box_lower(box: SevenBox) -> float:
        ...
        result = down_ratio(PRESSURE_NUMERATOR * low_prefix[-1], GRID * PRESSURE_DENOMINATOR)
    ...
    def convex_tangent_lower(box: SevenBox) -> Optional[arb]:
        ...
        value = sum((arb(point) for point in midpoints), arb(0)) * arb(fmpq(PRESSURE_NUMERATOR, PRESSURE_DENOMINATOR))
        gradient = [arb(fmpq(PRESSURE_NUMERATOR, PRESSURE_DENOMINATOR)) for _ in range(6)]
    ...
    return VerificationReport(
        ...
        verified=True,
        target="F6_q >= 341/100000 (q=29/100000)",
        grid=GRID,
