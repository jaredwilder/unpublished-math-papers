...
% Claude's direct contribution: Δ-closed-form + Pappalardi-controlled bound + empirical cancellation
...
\author{J.\ Wilder, with character-identity contribution from external GPT audit R682-R690\\
and $\Delta$-closed-form + Pappalardi-bound contribution from Claude (this work)}
\date{2026-05-12}
...
%% ===================================================================
\section{Lemma 1 --- Exact $\Gamma$-fiber formula (GPT R682)}
%% ===================================================================
...
%% ===================================================================
\section{Lemma 2 --- Box-period local count (GPT R683)}
%% ===================================================================
...
%% ===================================================================
\section{Lemma 3 --- Character expansion (GPT R684)}
%% ===================================================================
...
%% ===================================================================
\section{Lemma 4 (Claude's contribution) --- $\Delta_m(q)$ closed form and Pappalardi bound}
%% ===================================================================
...
%% ===================================================================
\section{Moment Laws for $\Delta_m(q)$ (GPT R691-R697 + Claude Lemmas 7-8)}
%% ===================================================================

\begin{lemma}[Exact second moment, GPT R691.2]\label{lem:R691-2}
For every prime $q \nmid 6$,
$$
...
\begin{corollary}[RMS bound, GPT R691.3]\label{cor:RMS-bound}
For every prime $q \nmid 6$,
$$
...
\begin{lemma}[CRT cross-moment vanishing, \textbf{Claude}]\label{lem:Claude-CRT}
...
Lemma~\ref{lem:Claude-CRT} verified for all 15 pairs $(q_1, q_2)$ with $q_1 \in \{5, 7, 11\}$, $q_2 \in \{13, 17, 19, 23, 29\}$: cross-moment exactly $0$ (all $|cm| \le 10^{-16}$). \hfill(File: \texttt{r691\_moment\_verify.json}.)
\end{remark}

\begin{lemma}[Variance of the discrepancy sum, \textbf{Claude}]\label{lem:Claude-var-sum}
...
Off-diagonal ($q_1 \neq q_2$) terms vanish by Lemma~\ref{lem:Claude-CRT}. Diagonal terms equal $\lambda_q^2 \cdot \mathbb{E}_m |\Delta_m(q)|^2 = \lambda_q^2 \cdot (1/(q-1))(1/|\Gamma_q| - 1/(q-1))$ by Lemma~\ref{lem:R691-2}.
...
\begin{corollary}[Cor 4.1$'$ density-zero via second moment, \textbf{Claude}]\label{cor:Claude-density-zero}
...
\begin{proof}
Markov's inequality applied to Lemma~\ref{lem:Claude-var-sum}: $\Pr_m[|\sum_q \Delta_m(q)| > T] \le \mathbb{E}_m |\sum_q \Delta_m(q)|^2 / T^2 = O(1/T^2)$.

The density-zero statement follows by integrating $T \to \infty$.

The Corollary 4.1$'$ derivation uses the second-moment NEG-203 sieve from Theorem 4.1$'$ of the manuscript (already in place, see Iwaniec-Kowalski Ch.~17 \S4). Inserting Lemma~\ref{lem:Claude-var-sum} as the variance input sharpens the explicit rate.
\end{proof}

%% ===================================================================
\section{Lemma 9 (\textbf{Claude}) --- Prime-only BV-Kummer reformulation as a Mertens-membership target}
%% ===================================================================

\begin{lemma}[Pure-prime fixed-$m$ BV-Kummer reformulation]\label{lem:Claude-Mertens}
...
\begin{lemma}[Sufficient cancellation for prime-modulus BV-Kummer, \textbf{Claude}]\label{lem:Claude-BV-sufficient}
...
\begin{remark}[Status of Lemma~\ref{lem:Claude-BV-sufficient} hypothesis]
...
\begin{lemma}[Squarefree extension sparsity, \textbf{Claude}]\label{lem:Claude-sqfree-sparse}
...
\begin{lemma}[\textbf{Claude Lemma 14} --- $\ell$-primary defect generalization]\label{lem:Claude-ell-primary}
...
\begin{remark}
\textbf{For $\ell = 2$}: local-full forces $u_i, v_i$ odd for all $i$, so both rows reduce to $(1, 1, \ldots, 1) \bmod 2$, giving $\rho_2 = 1$ and $\kappa_2(d) \ge 2^{r-1}$. This recovers Lemma~\ref{lem:Claude-2-torsion}.
...
%% ===================================================================
\section{Lemma 6 --- Squarefree modulus (GPT R686)}
%% ===================================================================
...
%% ===================================================================
\section{Theorem (the new exact Gate 0) --- GPT R687}
%% ===================================================================
...
\item Lemma~\ref{lem:Delta-closed} (Claude, $\Delta$-closed form): pointwise formula for $\Delta_m(q)$.
\item Lemma~\ref{lem:Delta-Pap} (Claude, Pappalardi-controlled): $|\Delta_m(q)| < q^{-1/2}$ on density-one primes.
\item Lemma~\ref{lem:Delta-avg} (Claude, $m$-average vanishes): $\mathbb{E}_m \Delta_m(q) = 0$.
...
Claude's contribution: Lemmas~\ref{lem:Delta-closed}, \ref{lem:Delta-Pap}, \ref{lem:Delta-avg}
