% Jet-Primitive Stepanov Auxiliary Theorem for the (2,3)-orbit
% J. Wilder, 2026.
...
\title{A Jet-Primitive Stepanov Auxiliary Theorem for the $(2,3)$-Orbit}
\author{Jared Wilder}
\date{2026-05-13}
...
follows the Bombieri Stepanov-method template adapted to the
multiplicative-additive 2-parameter orbit relevant to Erd\H{o}s--Graham
Problem \#203. In the specific obstruction problem studied here, the bound
$|B_q(c)| \ll \sqrt{h_q}\log q$ is in fact superseded by an exact
group-theoretic identity (proved in a companion note): $|B_q(c)| = \gcd(a_q,b_q)\cdot\mathbf{1}_{\Gamma_q}(c)$. We retain the Stepanov route here for two reasons: (i) it is methodologically independent and provides an unconditional upper bound that does not use the cyclicity of $\mathbb{F}_q^*$; (ii) the jet-primitive auxiliary-polynomial construction transfers to the more general setting where the orbit $V_q$ is not the underlying group itself.
\end{abstract}
...
$\log 7 \approx 1.95$) is handled by direct verification: at $q=7$, $D = 16$ gives kernel dimension $99$.
\end{remark}
...
direct numerical value $\delta_{\mathrm{emp}} = 0.4407$ at $q = 5003$,
$L = 80$ (reproduction code accompanies this note).
\end{theorem}

\begin{remark}[Empirical anchor]
...
$1/2$ within $8.6\%$.
\hfill\textit{(Numerical verification data accompanies this note.)}
\end{remark}
